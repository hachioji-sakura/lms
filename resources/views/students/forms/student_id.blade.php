  ...
  <div class="form-group">
    ...
    @if(isset($_edit) && $_edit===true)
      ...
      <div id="email-edit" class="w-100 email-edit" style="display:none;">
        <input type="text" id="student_id" name="email" class="form-control w-50 float-left" placeholder="任意の英数字"  required="true" inputtype="alnum"  minlength=3 maxlength=32 query_check="users/email" query_check_error="このログインIDはすでに使われています。別のIDを入力してください。" value="{{$item['email']}}">
    ...
    @else
      <input type="text" id="student_id" name="email" class="form-control" placeholder="任意の英数字" required="true" inputtype="alnum" query_check="users/email"  minlength=3 maxlength=32 query_check_error="このログインIDはすでに使われています。別のIDを入力してください。">
    @endif
  </div>
</div>
<script>
