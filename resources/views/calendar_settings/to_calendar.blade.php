...
$(function(){
  ...
  $("button.btn-submit").on('click', function(e){
    e.preventDefault();
    if(front.validateFormValue('calendar_settings_to_calendar .carousel-item.active')){
      $("form").submit();
    }
  });
  ...
  $('.carousel-item .btn-next').on('click', function(e){
    var form_data = front.getFormValue('calendar_settings_to_calendar');
    if(front.validateFormValue('calendar_settings_to_calendar .carousel-item.active')){
  ...
  $('.carousel-item .btn-prev').on('click', function(e){
    $('body, html').scrollTop(0);
    $('#calendar_settings_to_calendar').carousel('prev');
    $('#calendar_settings_to_calendar').carousel({ interval : false});
  });
  //確認画面用のパラメータ調整
