        ...
        <div class="form-group">
          ...
          <tr class="">
            ...
            <td class="p-1 text-center border-right" id="lesson_{{$week_code}}_time_{{$index}}_name">
              @if(isset($item) && isset($item->trial_students->first()->student->user) && $item->trial_students->first()->student->user->has_tag('lesson_'.$week_code.'_time', $index)===true)
                〇
              @else
                {{$item->trial_students->first()->student->user->has_tag('lesson_'.$week_code.'_time', $index)}}
              @endif
            </td>
            @endforeach
          </tr>
