...
trait Matching
{
  ...
  public function _candidate_teachers($teacher_id=0, $lesson=0){
    ...
    foreach($teachers as $teacher){
      ...
      $disable_subject = [];
      $enable_subject = [];
      $enable_places = $teacher->enable_places($this->type);
      $is_place_enable = false;
      foreach($this->tags->where('tag_key', 'lesson_place') as $tag){
        if($is_place_enable==false){
          $is_place_enable = $enable_places[intval($tag->tag_value)];
        }
      }
