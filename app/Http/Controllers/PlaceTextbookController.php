...
class PlaceTextbookController extends MilestoneController
{
  public $domain = 'place_textbooks';
  public $table = 'place_textbooks';
   ...
   * @return view / place_textbooks.lists
   */
  ...
  public function _store(Request $request)
  {
    ...
    return $this->transaction($request, function() use ($request){
      ...
      $item = Place::find($form['place_id']);
      $item->store_place_textbooks($form);
  ...
  public function search(Request $request)
  {
    ...
    $param = $this->get_param($request);
    //検索条件
    $items = $this->_search_scope($request, $param['place_textbooks']);
  ...
  public function get_param(Request $request, $id = null){
    ...
    $place_id = 0;
    if($request->has('place_id')) $place_id = $request->place_id;

    if(!empty($place_id)){
      $ret['place'] = Place::find($place_id);
      if(!isset($ret['place'])){
        abort(404, 'ページがみつかりません(1)');
      }
      $ret['place_id'] = $place_id;
      $ret['place_textbooks'] = $ret['place']->textbooks();
    } elseif (empty($id) && empty($place_id)){
      abort(400);
    }
