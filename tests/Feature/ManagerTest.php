...
class ManagerTest extends TestCase
{
     ...
     */
     ...
     {
       echo "\n----------ManagerTest::".__FUNCTION__." Start---------------";
     ...
     private function register()
     {
       ...
       $res =  $c->transaction($request, function() use ($request, $data1, $data2){
         $this->post('/managers/entry', $data1)->assertSessionHasAll(['error_message'=>'このメールアドレスはすでにユーザー登録済みです。']);
         echo "\n".$data1['email']."登録済みエラー : OK";

         $this->post('/managers/entry', $data2)->assertSessionHasAll(['success_message'=>'仮登録メールを送信しました。']);
         $this->assertDatabaseHas('common.users',  ['email'=>$data2['email']]);

         $t = Manager::where('name_last', $data2['name_last'])->where('name_first', $data2['name_first'])->first();
         echo "\n[register success!! for id=".$t->id."]";

         //Exceptionを発生させロールバックされる
         throw new \Exception("ManagerController::entry_store");
